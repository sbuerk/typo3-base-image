...
if [[ "$1" == "/usr/bin/supervisord" ]]; then
  ...
  adduser --home ${PROJECT_FOLDER} --no-create-home --disabled-login --ingroup typo3 --gecos GECOS --uid ${APP_RUN_USER} typo3;
...
if [[ "$( id -u )" -eq 0 ]]; then
  echo "Is root, dispatch entrypoint again with user typo3(${APP_RUN_USER})"
  su --group typo3 - typo3 -c "/usr/local/bin/entrypoint.sh $@"
  ...
  echo "Simply dispatch command $@ as root: $( id -u )"
  exec "$@"
fi